...
Session *TermWidgetImpl::createSession(QWidget *parent)
{
     ...
     * But as I'm not sure if you want to do anything else I'll just let both checks in and set this
     * to $SHELL anyway.
     */
...
void QTermWidget::search(bool forwards, bool next)
{
    ...
    regExp.setPatternOptions(m_searchBar->matchCase() ? QRegularExpression::NoPatternOption
                                                      : QRegularExpression::CaseInsensitiveOption);

    HistorySearch *historySearch = new HistorySearch(
        m_impl->m_session->emulation(), regExp, forwards, startColumn, startLine, this);
    connect(historySearch,
            SIGNAL(matchFound(int, int, int, int)),
            this,
    ...
    auto regexFilter =
        m_impl->m_terminalDisplay->filterChain()->getRegExpFilter(QLatin1String(QTERMW_HLIGHT));
    if (regexFilter) {
...
void QTermWidget::init(int startnow)
{
    ...
    for (const QString &dir : std::as_const(dirs)) {
        ...
        if (m_translator->load(QLocale::system(),
                               QLatin1String("qtermwidget"),
                               QLatin1String(QLatin1String("_")),
                               dir)) {
    ...
    connect(m_impl->m_session,
            SIGNAL(bellRequest(QString)),
    ...
    connect(m_impl->m_session,
            &Session::profileChangeCommandReceived,
            this,
    ...
    connect(
        m_impl->m_terminalDisplay, SIGNAL(copyAvailable(bool)), this, SLOT(selectionChanged(bool)));
    ...
    connect(m_impl->m_terminalDisplay,
            &TerminalDisplay::keyPressedSignal,
            this,
    ...
    connect(m_impl->m_session,
            &Session::currentDirectoryChanged,
            this,
...
void QTermWidget::setColorScheme(const QString &origName)
{
    ...
    if (!cs) {
        QMessageBox::information(
            this, tr("Color Scheme Error"), tr("Cannot load color scheme: %1").arg(name));
        return;
    }

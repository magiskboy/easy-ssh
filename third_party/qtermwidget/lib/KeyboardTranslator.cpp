...
void KeyboardTranslatorReader::readNext()
{
    ...
    while (!_source->atEnd()) {
        ...
        if (!tokens.isEmpty() && tokens.first().type == Token::KeyKeyword) {
            ...
            decodeSequence(
                tokens[1].text.toLower(), keyCode, modifiers, modifierMask, flags, flagMask);
...
KeyboardTranslator::Entry::Entry()
    : _keyCode(0), _modifiers(Qt::NoModifier), _modifierMask(Qt::NoModifier), _state(NoState),
      _stateMask(NoState), _command(NoCommand)
{
}

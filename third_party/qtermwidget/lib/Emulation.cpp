...
Emulation::Emulation()
    : _currentScreen(nullptr), _keyTranslator(nullptr), _usesMouse(false),
      _bracketedPasteMode(false), _toUtf16(QStringConverter::Utf8)
{
...
ScreenWindow *Emulation::createWindow()
{
    ...
    connect(this,
            &Emulation::handleCommandFromKeyboard,
            window,
...
//     a)  It decoded incoming characters one-by-one, which is slow in the current version of Qt
//     (4.2 tech preview) b)  It messed up decoding of non-ASCII characters, with the result that
...
uint ExtendedCharTable::createExtendedChar(uint *unicodePoints, ushort length)
{
    ...
    while (extendedCharTable.contains(hash) &&
           hash != 0) // 0 has a special meaning for chars so we don't use it
    {
        if (extendedCharMatch(hash, unicodePoints, length)) {
            ...
            if (hash == initialHash) {
                if (!triedCleaningSolution) {
                    ...
                    qWarning() << "Using all the extended char hashes, going to miss this extended "
                                  "character";
                    return 0;
                }
            }

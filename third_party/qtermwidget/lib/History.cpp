...
bool HistoryScrollFile::isWrappedLine(int lineno) const
{
    if (lineno >= 0 && lineno <= getLines()) {
        unsigned char flag;
        lineflags.get(
            (unsigned char *)&flag, sizeof(unsigned char), (lineno) * sizeof(unsigned char));
        return flag;
    }
    return false;
}
...
HistoryScrollBuffer::HistoryScrollBuffer(unsigned int maxLineCount)
    : HistoryScroll(new HistoryTypeBuffer(maxLineCount)), _historyBuffer(), _maxLineCount(0),
      _usedLines(0), _head(0)
{
...
void HistoryScrollBuffer::getCells(int lineNumber,
                                   int startColumn,
                                   int count,
                                   Character buffer[]) const
{
    if (count == 0)
        return;
    ...
    : blockList(bList), formatLength(0)
{
    ...
    if (!line.empty()) {
        ...
        formatArray[0].startPos =
            0; // there's always at least 1 format (for the entire line, unless a change happens)
...
void CompactHistoryScroll::getCells(int lineNumber,
                                    int startColumn,
                                    int count,
                                    Character buffer[]) const
{

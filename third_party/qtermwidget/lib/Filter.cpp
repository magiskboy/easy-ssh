    ...
    for (int i = 0; i < _linePositions->count(); i++) {
        ...
        if (_linePositions->value(i) <= position && position < nextLine) {
            ...
            startColumn = string_width(
                buffer()
                    ->mid(_linePositions->value(i), position - _linePositions->value(i))
                    .toStdWString());
            return;
        }
    }
...
Filter::HotSpot::HotSpot(int startLine, int startColumn, int endLine, int endColumn)
    : _startLine(startLine), _startColumn(startColumn), _endLine(endLine), _endColumn(endColumn),
      _type(NotSpecified)
{
}
...
void RegExpFilter::process()
{
    ...
    auto match = _searchText.match(emptyString,
                                   0,
...
void RegExpFilter::newHotSpot(
    int startLine, int startColumn, int endLine, int endColumn, const QStringList &captureList)
{
...
void UrlFilter::newHotSpot(
    int startLine, int startColumn, int endLine, int endColumn, const QStringList &captureList)
{
    // Use the old hotspot if existing.
    for (const auto &hs : std::as_const(_oldHotspotList)) {
        if (hs->startLine() == startLine && hs->endLine() == endLine &&
            hs->startColumn() == startColumn && hs->endColumn() == endColumn) {
...
// regexp matches:
//  full url:
//  NOTE: It is supposed that a URL does not end with a punctuation mark, parenthesis, bracket or
//  single-quotation mark.
const QRegularExpression UrlFilter::FullUrlRegExp(
    QLatin1String("[A-Za-z0-9_\\-]+://((?!&quot;|&gt;|&lt;)[A-Za-z0-9_.+/"
                  "\\?\\=~&%#,;!@\\*\'\\-:\\(\\)\\[\\]])+(?<!\\.|\\?|!|:|;|,|\\(|\\)|\\[|\\]|\')"));
// email address:
...
const QRegularExpression
    UrlFilter::CompleteUrlRegExp(QLatin1Char('(') + FullUrlRegExp.pattern() + QLatin1Char('|') +

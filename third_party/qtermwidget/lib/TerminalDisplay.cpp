...
// the user doesn't really touch the mouse - once the mouse moves we'll quickly be out of the
// deadzone
...
void TerminalDisplay::fontChange(const QFont &)
{
    ...
    _fontWidth = qRound(static_cast<double>(fm.horizontalAdvance(QLatin1String(REPCHAR))) /
                        static_cast<double>(qstrlen(REPCHAR)));
...
void TerminalDisplay::calDrawTextAdditionHeight(QPainter &painter)
{
    ...
    painter.drawText(
        test_rect, Qt::AlignBottom, LTR_OVERRIDE_CHAR + QLatin1String("Mq"), &feedback_rect);
    painter.restore();
...
TerminalDisplay::TerminalDisplay(QWidget *parent)
    : QWidget(parent), _screenWindow(nullptr), _allowBell(true), _gridLayout(nullptr),
      _fontHeight(1), _fontWidth(1), _fontAscent(1), _boldIntense(true), _lines(1), _columns(1),
      _usedLines(1), _usedColumns(1), _contentHeight(1), _contentWidth(1), _image(nullptr),
      _randomSeed(0), _resizing(false), _terminalSizeHint(false), _terminalSizeStartup(true),
      _bidiEnabled(true), _mouseMarks(false), _disabledBracketedPasteMode(false), _actSel(0),
      _wordSelectionMode(false), _lineSelectionMode(false), _preserveLineBreaks(false),
      _columnSelectionMode(false), _scrollbarLocation(QTermWidget::NoScrollBar),
      _wordCharacters(QLatin1String(":@-./_~")), _bellMode(SystemBeepBell), _blinking(false),
      _hasBlinker(false), _cursorBlinking(false), _hasBlinkingCursor(false),
      _allowBlinkingText(true), _ctrlDrag(false), _tripleClickMode(SelectWholeLine),
      _isFixedSize(false), _possibleTripleClick(false), _resizeWidget(nullptr),
      _resizeTimer(nullptr), _flowControlWarningEnabled(false), _outputSuspendedLabel(nullptr),
      _lineSpacing(0), _colorsInverted(false), _opacity(static_cast<qreal>(1)),
      _backgroundMode(None), _filterChain(new TerminalImageFilterChain()),
      _cursorShape(Emulation::KeyboardCursorShape::BlockCursor),
      mMotionAfterPasting(NoMoveScreenWindow), _leftBaseMargin(1), _topBaseMargin(1),
      _drawLineChars(true), _mouseAutohideDelay(-1)
{
...
void TerminalDisplay::drawLineCharString(
    QPainter &painter, int x, int y, const std::wstring &str, const Character *attributes) const
{
...
void TerminalDisplay::drawBackground(QPainter &painter,
                                     const QRect &rect,
                                 ...
                                 bool &invertCharacterColor)
{
    ...
    if (!_cursorBlinking) {
        ...
        if (_cursorShape == Emulation::KeyboardCursorShape::BlockCursor) {
            ...
            painter.drawLine(
                cursorRect.left(), cursorRect.bottom(), cursorRect.right(), cursorRect.bottom());
        else if (_cursorShape == Emulation::KeyboardCursorShape::IBeamCursor)
            painter.drawLine(
                cursorRect.left(), cursorRect.top(), cursorRect.left(), cursorRect.bottom());
    }
}
    ...
    if (isLineCharString(text))
        ...
        if (_bidiEnabled) {
            if (tooWide) {
                ...
                painter.drawText(
                    rect.x(), rect.y() + _fontAscent + _lineSpacing, QString::fromStdWString(text));
            }
        } else {
            QRect drawRect(rect.topLeft(), rect.size());
            drawRect.setHeight(rect.height() + _drawTextAdditionHeight);
            painter.drawText(
                drawRect, Qt::AlignBottom, LTR_OVERRIDE_CHAR + QString::fromStdWString(text));
        }
    }
}
        ...
        if (!_resizing) // not while _resizing, we're expecting a paintEvent
            for (x = 0; x < columnsToUpdate; ++x) {
                ...
                if (dirtyMask[x]) {
                    ...
                    int charWidth = QChar::requiresSurrogates(static_cast<char32_t>(c))
                                        ? fm.horizontalAdvance(QString::fromWCharArray(&c, 1))
                                        : fm.horizontalAdvance(QChar(static_cast<char16_t>(c)));
                    ...
                    for (len = 1; len < lln; ++len) {
                        ...
                        wchar_t nxtCh = newLine[x + len].character;
                        int nxtCharWidth =
                            QChar::requiresSurrogates(static_cast<char32_t>(nxtCh))
                                ? fm.horizontalAdvance(QString::fromWCharArray(&nxtCh, 1))
                                : fm.horizontalAdvance(QChar(static_cast<char16_t>(nxtCh)));
                        bool nextIsbigWidth =
                            _fixedFont && !nextIsDoubleWidth && nxtCharWidth > _fontWidth;
                        bool nextIsSmallWidth =
                            _fixedFont && newLine[x + len].character && nxtCharWidth < _fontWidth;

                        if (ch.foregroundColor != cf || ch.backgroundColor != _clipboard ||
                            ch.rendition != cr || !dirtyMask[x + len] ||
                            isLineChar(ch) != lineDraw || nextIsDoubleWidth != doubleWidth ||
                            bigWidth || nextIsbigWidth || smallWidth || nextIsSmallWidth) {
                            break;
                        }
    ...
    if (_terminalSizeHint && isVisible()) {
        ...
        if (!_resizeWidget) {
            ...
            _resizeWidget->setStyleSheet(
                QLatin1String("background-color:palette(window);border-style:solid;border-width:"
                              "1px;border-color:palette(dark)"));
...
void TerminalDisplay::paintFilters(QPainter &painter)
{
    ...
    while (iter.hasNext()) {
        ...
        for (int line = spot->startLine(); line <= spot->endLine(); line++) {
            ...
            while (QChar::isSpace(static_cast<char32_t>(_image[loc(endColumn, line)].character)) &&
                   endColumn > 0)
                endColumn--;
...
int TerminalDisplay::textWidth(const int startColumn, const int length, const int line) const
{
    ...
    for (int column = 0; column < length; column++) {
        ...
        if (_fixedFont_original &&
            !isLineChar(c)) { // c == 0 may happen here after a double-column character
            result += fm.horizontalAdvance(QLatin1Char(REPCHAR[0]));
        } else {
            const char32_t ucs4 = static_cast<char32_t>(c.character);
            result += QChar::requiresSurrogates(ucs4)
                          ? fm.horizontalAdvance(QString::fromUcs4(&ucs4, 1))
                          : fm.horizontalAdvance(QChar(static_cast<char16_t>(ucs4)));
        }
    }
    return result;
}

QRect TerminalDisplay::calculateTextArea(
    int topLeftX, int topLeftY, int startColumn, int line, int length)
{
    ...
    for (int y = luy; y <= rly; y++) {
        ...
        for (; x <= rlx; x++) {
            ...
            if (_image[loc(x, y)].rendition & RE_EXTENDED_CHAR) {
                ...
                uint *chars = ExtendedCharTable::instance.lookupExtendedChar(
                    _image[loc(x, y)].character, extendedCharLength);
                if (chars) {
            ...
            int charWidth = QChar::requiresSurrogates(static_cast<char32_t>(c))
                                ? fm.horizontalAdvance(
                                      QString::fromUcs4(reinterpret_cast<const char32_t *>(&c), 1))
                                : fm.horizontalAdvance(QChar(static_cast<char16_t>(c)));
            bool bigWidth = _fixedFont && !doubleWidth && charWidth > _fontWidth;
            ...
            while (x + len <= rlx && _image[loc(x + len, y)].foregroundColor == currentForeground &&
                   ...
                   (nxtDoubleWidth = (_image[qMin(loc(x + len, y) + 1, _imageSize)].character ==
                                      0)) == doubleWidth &&
                   !smallWidth &&
                   !(_fixedFont && (nxtC = _image[loc(x + len, y)].character) &&
                     (nxtCharWidth = (QChar::requiresSurrogates(static_cast<char32_t>(nxtC))
                                          ? fm.horizontalAdvance(QString::fromUcs4(
                                                reinterpret_cast<const char32_t *>(&nxtC), 1))
                                          : fm.horizontalAdvance(QChar(
                                                static_cast<char16_t>(nxtC))))) < _fontWidth) &&
                   !bigWidth &&
                   ...
                   isLineChar(_image[loc(x + len, y)]) == lineDraw) // Assignment!
            {
                ...
                if (_image[loc(x + len, y)].rendition & RE_EXTENDED_CHAR) {
                    ...
                    ushort extendedCharLength = 0;
                    const uint *chars =
                        ExtendedCharTable::instance.lookupExtendedChar(c, extendedCharLength);
                    if (chars) {
                        Q_ASSERT(extendedCharLength > 1);
                        bufferSize += extendedCharLength - 1;
                        unistr.resize(bufferSize);
...
void TerminalDisplay::mousePressEvent(QMouseEvent *ev)
{
    ...
    if (ev->button() == Qt::LeftButton) {
        ...
        if ((!_ctrlDrag || ev->modifiers() & Qt::ControlModifier) && selected) {
            ...
            _preserveLineBreaks =
                !((ev->modifiers() & Qt::ControlModifier) && !(ev->modifiers() & Qt::AltModifier));
        ...
        if (_mouseMarks || (ev->modifiers() & Qt::ShiftModifier))
            ...
            emit mouseSignal(
                1, charColumn + 1, charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 0);
    } else if (ev->button() == Qt::RightButton) {
        if (_mouseMarks || (ev->modifiers() & Qt::ShiftModifier))
            emit configureRequest(ev->pos());
        else
            emit mouseSignal(
                2, charColumn + 1, charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 0);
    }
}
...
void TerminalDisplay::hideStaleMouse() const
{
    ...
    if (QApplication::activeWindow() &&
        QApplication::activeWindow() != window()) // some other app window has the focus
        return;
...
void TerminalDisplay::mouseMoveEvent(QMouseEvent *ev)
{
    ...
    if (!_mouseMarks && !(ev->modifiers() & Qt::ShiftModifier)) {
        ...
        emit mouseSignal(
            button, charColumn + 1, charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 1);
    ...
    if (!_wordSelectionMode && !_lineSelectionMode) {
        ...
        if (right.x() > 0 && !_columnSelectionMode) {
            ...
            if (i >= 0 && i <= _imageSize) {
                ...
                /* if (selClass == ' ')
                 {
                   while ( right.x() < _usedColumns-1 && charClass(_image[i+1]) == selClass &&
...
void TerminalDisplay::mouseDoubleClickEvent(QMouseEvent *ev)
{
    ...
    char32_t selClass = charClass(_image[i]);
    {
        ...
        if (_image[i].character == L'@' && endSel.x() - bgnSel.x() > 0 &&
...
void TerminalDisplay::mouseTripleClickEvent(QMouseEvent *ev)
{
    ...
    if (_tripleClickMode == SelectForwardsFromCursor) {
        ...
        while (
            ((x > 0) || (_iPntSel.y() > 0 && (_lineProperties[_iPntSel.y() - 1] & LINE_WRAPPED))) &&
            charClass(_image[i - 1]) == selClass) {
            i--;
            if (x > 0)
                x--;
...
char32_t TerminalDisplay::charClass(const Character &ch) const
{
    if (ch.rendition & RE_EXTENDED_CHAR) {
        ushort extendedCharLength = 0;
        const uint *chars =
            ExtendedCharTable::instance.lookupExtendedChar(ch.character, extendedCharLength);
        if (chars && extendedCharLength > 0) {
            ...
            return allLetterOrNumber ? U'a' : chars[0];
        }
        return 0;
    } else {
        if (QChar::isSpace(static_cast<char32_t>(ch.character)))
            return U' ';
        if (QChar::isLetterOrNumber(static_cast<char32_t>(ch.character)) ||
            _wordCharacters.contains(QString::fromWCharArray(&ch.character, 1),
                                     Qt::CaseInsensitive))
            return U'a';
        return static_cast<char32_t>(ch.character);
    }
}
...
void TerminalDisplay::outputSuspended(bool suspended)
{
    ...
    if (!_outputSuspendedLabel) {
        ...
        _gridLayout->addItem(
            new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Expanding), 1, 0);
    }

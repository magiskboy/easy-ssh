...
Session::Session(QObject *parent)
    : QObject(parent), _shellProcess(nullptr), _emulation(nullptr), _monitorActivity(false),
      _monitorSilence(false), _notifiedActivity(false), _autoClose(true), _wantedClose(false),
      _silenceSeconds(10), _isTitleChanged(false),
      ...
      _hasDarkBackground(false)
{
    ...
    connect(
        _emulation, SIGNAL(imageResizeRequest(QSize)), this, SLOT(onEmulationSizeChange(QSize)));
    ...
    connect(_shellProcess,
            SIGNAL(receivedData(const char *, int)),
            this,
...
QString Session::currentWorkingDirectory()
{
    if (_reportedWorkingUrl.isValid() && _reportedWorkingUrl.isLocalFile() &&
        (_reportedWorkingUrl.host().length() == 0 ||
         _reportedWorkingUrl.host().compare(QSysInfo::machineHostName(), Qt::CaseInsensitive) ==
             0)) {
...
void Session::addView(TerminalDisplay *widget)
{
    ...
    if (_emulation != nullptr) {
        ...
        connect(widget,
                SIGNAL(mouseSignal(int, int, int, int)),
                _emulation,
                SLOT(sendMouseEvent(int, int, int, int)));
        connect(widget,
                SIGNAL(sendStringToEmu(const char *)),
                _emulation,
        ...
        connect(
            _emulation, SIGNAL(programUsesMouseChanged(bool)), widget, SLOT(setUsesMouse(bool)));

        widget->setUsesMouse(_emulation->programUsesMouse());

        connect(_emulation,
                SIGNAL(programBracketedPasteModeChanged(bool)),
                widget,
    ...
    QObject::connect(
        widget, SIGNAL(changedContentSizeSignal(int, int)), this, SLOT(onViewSizeChange(int, int)));
...
void Session::run()
{
    ...
    /* ok I'm not exactly sure where _program comes from - however it was set to /bin/bash on my
     * system That's bad for BSD as its /usr/local/bin/bash there - its also bad for arch as its
     ...
     */
    int result = _shellProcess->start(
        exec, arguments, _environment << backgroundColorHint, windowId(), _addToUtmp);

...
# CMake on Windows expects ';' in CMAKE_PREFIX_PATH; shell PATH still uses ':'.
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*) CMAKE_PATH_SEP=';' ;;
  *) CMAKE_PATH_SEP=':' ;;
esac

mkdir -p "$BUILD_ROOT" "$PREFIX"
export CMAKE_PREFIX_PATH="${PREFIX}${CMAKE_PATH_SEP}${CMAKE_PREFIX_PATH}"
